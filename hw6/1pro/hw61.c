...
int main(int argc,char **args)
{
  ...
  //PetscScalar    one = 1.0,value[3],r,lambda;
  ...
  */
  ...
  //ierr = VecDuplicate(z,&zlag);CHKERRQ(ierr);
  ...
  */
  ...
  //ierr = VecNorm(y,NORM_2,&r);CHKERRQ(ierr);
  //ierr = VecCopy(y,zlag);CHKERRQ(ierr);
  //scalar = one/r;
  //ierr = VecScale(zlag,scalar);CHKERRQ(ierr);
  //ierr = VecCopy(zlag,z);CHKERRQ(ierr);
  //ierr = MatMult(A,z,ylag);CHKERRQ(ierr);
  
  //ierr = MatMult(A,z,ylag);CHKERRQ(ierr);
  //ierr = VecAXPY(ylag,-1.0,y);CHKERRQ(ierr);
  //ierr = VecNorm(ylag,NORM_2,&norm);CHKERRQ(ierr);
  //ierr = VecAXPY(norm,-1.0,r);CHKERRQ(ierr);
  //norm = fabs(norm - r);
